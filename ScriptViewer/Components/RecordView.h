...
class RecordView : public Component, public ScriptRoutineTracer
{
	...
	class DetailedScriptProfile
	{
	  private:
		class ScriptTrace
		{
		  ...
		  public:
			ScriptTrace(DWORD_t dwIP, DWORD64 qwExecTimeMcS = 0)
			    : m_dwScriptIP(dwIP), m_qwScriptExecTimeMcS(qwExecTimeMcS)
			{
			}

			ScriptTrace(const ScriptTrace &scriptTrace)
			    : ScriptTrace(scriptTrace.m_dwScriptIP, scriptTrace.m_qwScriptExecTimeMcS)
			{
			...
			ScriptTrace(ScriptTrace &&scriptTrace) noexcept
			    : ScriptTrace(scriptTrace.m_dwScriptIP, scriptTrace.m_qwScriptExecTimeMcS)
			{
				m_fScriptExecTimeSecs = scriptTrace.m_fScriptExecTimeSecs;
				m_bScriptHasEnded     = scriptTrace.m_bScriptHasEnded;
			...
			inline void End()
			{
				...
				m_fScriptExecTimeSecs =
				    !m_qwScriptExecTimeMcS ? m_qwScriptExecTimeMcS : m_qwScriptExecTimeMcS / 1000000.f;
			}
		...
		mutable std::set<std::unique_ptr<ScriptTrace>,
		                 decltype([](const std::unique_ptr<ScriptTrace> &pLeftScriptTrace,
		                             const std::unique_ptr<ScriptTrace> &pRightScriptTrace)
	  ...
	  public:
		...
		DetailedScriptProfile(const DetailedScriptProfile &scriptProfile)
		    : DetailedScriptProfile(scriptProfile.m_szScriptName, scriptProfile.m_eTraceMethod,
		                            scriptProfile.m_bIsCustomScript)
		{
		...
		inline void SendMsg(EMessageType eMessageType, DWORD64 qwData = 0)
		{
			...
			else if (!m_rgScriptRoutineTraces.empty())
			{
				...
				else if (eMessageType == EMessageType::LeaveCalled)
				{
					...
					{
						m_dictScriptTraces.insert(std::make_pair(
						    scriptTrace.GetIP(),
						    std::make_unique<ScriptRoutineTrace>(scriptTrace.GetIP(), scriptTrace.GetMcS())));
					}
					else
					{
  ...
  public:
	...
	virtual void ScriptRoutineCallback(rage::scrThread *pThread, ERoutineTraceType eTraceType,
	                                   DWORD_t dwEnterIP) override;
};